...
APA clustering, safe GTF-based annotation, ZT-tagged per-isoform BAMs,
and a detailed classification summary TSV.

What's new vs v3
----------------
• Writes <out>_classification_summary.tsv summarizing, per isoform:
  - isoform intron chain (tx-order) & TES
  - matched GTF transcript intron chain & TES
  - TES delta (bp), exon overlap (bp), match source (TES_TOL / OVERLAP / NONE)
  - final classification (EXACT / NOVEL_APA / NOVEL_CHAIN / NOVEL_LOCUS)
  - read support stats and per-sample counts
• Keeps all v3 behavior (APA clustering with mode TES, suffix-based 5' collapse,
  polyA gating, safe TES-tolerant GTF annotation, ZT-tag BAMs, modkit manifest).
"""
...
def annotate_isoform(iso, gtf_txs, tes_match_tol):
    """Return annotation dict including classification and a few diagnostics."""
    chrom, strand, tes = iso["chrom"], iso["strand"], iso["tes"]
    iso_chain = iso["chain_tx"]
    iso_exons = iso["rep_exons"]

    # First, candidates by TES tolerance (same chr/strand & TES within tol)
    ...
    def rank(tx):
        ...
        same_chain = 0 if tx.chain_tx == iso_chain else 1
        return (abs(tx.tes_1based - tes), same_chain, -exon_overlap_len(iso_exons, tx.exons), -len(tx.chain_tx))

    matched = None
    match_source = "NONE"
    if cands:
        matched = min(cands, key=rank)
        match_source = "TES_TOL"
    else:
        # overlap fallback on same chr/strand
        overlaps = [tx for tx in gtf_txs if tx.chrom==chrom and tx.strand==strand
                    and exon_overlap_len(iso_exons, tx.exons) > 0]
        if overlaps:
            matched = max(overlaps, key=lambda tx: exon_overlap_len(iso_exons, tx.exons))
            match_source = "OVERLAP"

    if matched:
        tes_delta = abs(matched.tes_1based - tes)
        ov_bp = exon_overlap_len(iso_exons, matched.exons)
        if matched.chain_tx == iso_chain:
            cls = "EXACT" if tes_delta == 0 else "NOVEL_APA"
        else:
            cls = "NOVEL_CHAIN"
        return dict(
            classification=cls,
            gene_id=matched.gene_id,
            gene_name=matched.gene_name,
            matched_tid=matched.transcript_id,
            matched_tes=matched.tes_1based,
            matched_chain=matched.chain_tx,
            tes_delta=tes_delta,
            exon_overlap_bp=ov_bp,
            match_source=match_source
        )
    else:
        return dict(
            classification="NOVEL_LOCUS",
            gene_id="NA", gene_name="NA",
            matched_tid="NA", matched_tes="NA",
            matched_chain=tuple(),
            tes_delta="NA", exon_overlap_bp=0,
            match_source="NONE"
        )
...
def main():
    ap = argparse.ArgumentParser(description="TES/APA assembler with GTF annotation, ZT tagging, and classification summary")
    # Inputs
    ...
    # Annotation + codes + classification summary rows
    summary_rows = []
    for iso in kept:
        if gtf_txs:
            ...
            ann = dict(classification="NOVEL_LOCUS", gene_id="NA", gene_name="NA", matched_tid="NA",
                       matched_tes="NA", matched_chain=tuple(), tes_delta="NA", exon_overlap_bp=0, match_source="NONE")
        iso["annotation"] = ann
        ...
        summary_rows.append([
            iso["code"],
            iso["chrom"], iso["strand"],
            iso["tes"], chain_to_str(iso["chain_tx"]),
            ann["matched_gid"] if "matched_gid" in ann else ann["gene_id"],
            ann["gene_name"], ann["matched_tid"],
            ann["matched_tes"],
            chain_to_str(ann["matched_chain"]),
            ann["tes_delta"], ann["exon_overlap_bp"], ann["match_source"],
            ann["classification"],
            iso["count"], f"{iso['frac_global']:.4f}", f"{iso['polya_frac']:.4f}",
    ...
    summary_path = args.out_gtf.replace(".gtf","") + "_classification_summary.tsv"
    with open(summary_path, "w") as f:
        f.write("#code\tchrom\tstrand\tiso_tes\tiso_chain_tx\tgtf_gene_id\tgtf_gene_name\tgtf_transcript_id\tgtf_tes\tgtf_chain_tx\ttes_delta_bp\texon_overlap_bp\tmatch_source\tclassification\tread_support\tfrac_global\tpolya_support_frac\tsample_counts\n")
    ...
    print(f"[OK] Wrote {args.out_gtf}, metrics {metrics_path}, and summary {summary_path}", file=sys.stderr)
