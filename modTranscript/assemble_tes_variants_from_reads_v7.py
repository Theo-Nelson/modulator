#!/usr/bin/env python3
"""
Read-backed TES/APA assembler (v7)

Purpose
-------
Make one ZT/ZN-tagged copy of each input BAM (same count as inputs) so you can run
modkit with `--partition-tag ZN` (ZN = transcript index within a gene, 1..k).
This yields far fewer modkit files (one per transcript index across all genes).
GTF + summary TSV let you map back to gene/transcript identities.

Tags written on assigned reads:
  ZN (i): transcript_index within its gene (1..k)
  ZG (i): gene_index (1..G) – deterministic per run
  ZT (Z): '{gene_name}.{gene_id}.G{ZG}.T{ZN}'

Outputs:
  • <out>.gtf (with zt_label, gene_index, transcript_index)
  • <out>_classification_summary.tsv (zt_label, gene_index, transcript_index, …)
  • zt_tagged/<sample>.zt_tagged.bam  (if --write-zt-tagged-sample-bams; count == input BAMs)
  • zt_bams/<sample>.<ZT>.bam         (optional per-transcript subsets if --write-zt-bams)
"""
...
# ---------- main ----------
def main():
    ap = argparse.ArgumentParser(description="TES/APA assembler v7 with ZT/ZG/ZN tags (ZN = transcript index within gene)")
    # Inputs
    ...
    ap.add_argument("--tes-window", type=int, default=None)  # alias
    ...
    for (chrom, strand), cluster_list in groups.items():
        for cl in cluster_list:
            ...
            sorted_chains = sorted(chain_counts.keys(), key=lambda ch: (len(ch), chain_counts[ch]), reverse=True)
            assigned = set()
            ...
            for canon in sorted_chains:
                ...
                rep = max(full_len_members or members, key=lambda m: (m["exons"][-1][1]-m["exons"][0][0]))
    ...
    # Optional: per-transcript subsetted BAMs
    if args.write_zt_bams or args.emit-modkit-manifest:
        ...
        for iso in kept:
            ...
            for sample, n in iso["sample_ct"].items():
                if n < args.min_reads_per_sample_for_mod: continue
            ...
            with pysam.AlignmentFile(bam, "rb") as inp:
                ...
                try:
                    ...
                    for aln in inp.fetch(region=args.region) if args.region else inp.fetch():
                        ...
                        if not tup: continue
                        zt, zg, zn = tup
                        try: aln.set_tag("ZT", zt, value_type="Z", replace=True)
                        except TypeError: aln.set_tag("ZT", zt)
                        for tag, val in (("ZG", int(zg)), ("ZN", int(zn))):
                            try: aln.set_tag(tag, val, value_type="i", replace=True)
                            except TypeError: aln.set_tag(tag, val)
    ...
    if args.write_zt_tagged_sample_bams:
        ...
        for bam in bams:
            ...
            with pysam.AlignmentFile(in_path, "rb") as inp, pysam.AlignmentFile(out_path, "wb", header=inp.header) as outw:
                for aln in inp.fetch(region=args.region) if args.region else inp.fetch():
                    if aln.is_unmapped:
