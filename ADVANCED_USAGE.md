...
| `snp_scan_max_depth` | int | `1000` | Per-sample depth cap in SNP discovery: a `snp_scan_depth_chunk_bp` (5 kb) chunk overlapped by more reads is scanned from a deterministic read-name-hash subsample, so allele fractions stay unbiased while one ultra-deep locus (chrM, rRNA, a top-expressed gene) cannot hold a worker for hours. `0` = exact counts. Only the candidate list is affected; per-molecule tables use every read. The scan also checkpoints every window under `results/tmp/<prefix>/snp_scan_shards` so a restarted stage resumes instead of rescanning. |
...
| `min_mod_site_nmod` / `min_mod_site_frac` / `min_mod_site_frac_cov` | int / float / int | `1` / `0` / `20` | Candidate-site evidence floors: pooled modified reads (1 = drop positions with no modified call), and optionally at least one (sample × fragmentform) row with coverage ≥ `min_mod_site_frac_cov` and modified fraction ≥ `min_mod_site_frac`. The ZN table is thresholded, so its fractions run below per-read call rates; a 0.10 floor cost ~9 % of HG002's significant pairs, hence off by default. |
| `mod_table_row_budget` | int | `400000000` | Row budget for the per-read modification table (rows ≈ 2 × summed site coverage, ~225 B/row → ~90 GB). Runs that fit keep every site; a cohort that would exceed it keeps sites in order of evidence (strongest per-row modified fraction, then pooled modified reads) until the budget is filled. The 31-library mouse cohort projects to ~9 G rows (~2 TB) unbounded. 0 = no budget. |
| `molecule_table_format` | `parquet` / `tsv` | `parquet` | On-disk format of the two per-read tables (`*_molecule_mod_calls`, `*_molecule_snps`). Parquet: one row group per chromosome block, zstd + dictionary encoding (~5–10× smaller than the TSV, several-fold faster to write and to read one chromosome). Every consumer reads either format through `open_chrom_table`; `workflow/scripts/convert_table_format.py --in X.tsv --out X.parquet` converts existing tables either way. |
| `mod_site_max_snp_distance_bp` | int | `0` | Optional cis window: keep only modification sites within this distance of a candidate SNP (0 = any SNP-linked site). |
| `max_reads_per_mod_site` | int | `1000` | Per-sample cap on reads emitted per site in the per-read modification table (deterministic read-name-hash subsample when the site's expected per-sample depth exceeds it; modified fractions unbiased). 0 = every read. The extraction is checkpointed per (sample × chromosome) under `results/tmp/<prefix>/molmod_shards`, so a requeued or restarted stage resumes instead of re-extracting. |
