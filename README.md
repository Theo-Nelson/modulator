...
- TES clustering now uses direct distal 5' splice-junction support to rank canonicals instead of always letting the longest compatible chain absorb shorter suffixes.
- `ZN` is now assigned with metagene-aware partition reuse so non-overlapping transcripts inside one overlapping metagene can share a partition index without throwing overlapping-locus reads away.
- The multigene stage now retains and summarizes overlapping reads instead of discarding them.
...
Package-native quick demos on the bundled datasets:
...
modulator demo \
  --reference-fa /path/to/ref.fa \
  --reference-gtf /path/to/ref.gtf \
  --dataset ALCAM_NHSL1_SERAC1_MXD1_RIOK3_reads \
  --mode full
```
...
modulator demo --reference-fa /path/to/ref.fa --reference-gtf /path/to/ref.gtf --dataset RPL13_reads
...
- `RPL13_reads` and `ALCAM_NHSL1_SERAC1_MXD1_RIOK3_reads` are exposed as separate bundled demo datasets for single-gene and five-gene walkthroughs.
...
| `min_distal_anchor_reads` | integer | `2` | `1–20` | Minimum number of exact-chain reads needed before a longer chain can absorb shorter suffix-compatible chains under the default support-first assignment policy. |
| `min_distal_anchor_frac` | float | `0.05` | `0.0–1.0` | Minimum exact-chain fraction within a suffix family required for longer-chain absorption under the default support-first assignment policy. |
...
- **Support-first assignment:** a longer suffix-compatible chain can absorb shorter chains only when it has direct exact-chain support from its own distal unique 5′ splice structure.  
...
- **ZN semantics:** `ZN` is a metagene-aware partition index rather than a simple transcript index within one gene.

### Multigene Filter Parameters

This optional post-assembly module scans each `zt_tagged` BAM, summarizes overlap against assembled gene exon unions, and writes a cleaned BAM for downstream `modkit`. Overlapping reads are retained and categorized instead of being discarded.
...
|---|---|---:|---|
| `zn` | `partition_tag` | `"ZN"` | Partition by ZN, using the metagene-aware partition index written by the assembler |
...
### Granular Site Classification Parameters (`classify_diffs`)

This stage runs **after** `test_diffs`. For every significant between-isoform site (passing `fdr` **and** `min_effect`, i.e. the ">10% absolute stoichiometry" rule) it assigns one MECE structural category explaining *why* the isoforms differ in modification stoichiometry there, anchored to the gene's **longest-3'UTR** isoform. It covers **all detected modifications** (every `mod_code` carried by the diff table — m6A `a`, 5mC `m`, pseudouridine, etc.), not just m6A. It reads `{prefix}__ZN_site_diff_results.tsv` + the assembled `{prefix}.gtf` and writes `{prefix}__ZN_site_classified.tsv`.

| Key | Type | Default | Description |
|---|---|---|---|
| `classify_diffs.enable` | bool | `true` | Run the classification stage. |
| `classify_diffs.min_effect` | float | `0.10` | Minimum effect = max \|Δ stoichiometry\| (the >10% absolute rule). |
| `classify_diffs.fdr` | float | `0.05` | Maximum BH-FDR (`p_adj_bh`). |
| `classify_diffs.mod_filter` | list[str] / null | `null` | mod_code(s) to classify. `null` → **all detected modifications** (inherits `test_diffs.mod_filter` if that is set; otherwise no filter). Set e.g. `["a"]` to restrict to m6A only. |
| `classify_diffs.min_cov` | int | `0` | Extra per-isoform `Ncov` floor on the JSON entries (the diff table is already coverage-filtered by `test_diffs.min_cov`). |
| `classify_diffs.tes_tol` | int | `200` | TES match tolerance for architecture/APA calls. |
| `classify_diffs.inside_tol` | int | `50` | Last-exon acceptor match tolerance. |
| `classify_diffs.ejc_nt` | int | `150` | EJC suppression zone (nt) for `SPLICING_EJC` / `IPA_SHARED_EJC`. |
| `classify_diffs.intergenic_gap` | int | `1000` | Minimum genomic gap (bp) between disjoint terminal exons to call `INTERGENIC_TERMINAL_EXON`. |
| `classify_diffs.figures` | bool | `true` | Render the per-category figures: **(1)** isoform **architecture maps** (`__figs_by_category_arch/`) — every isoform of the gene drawn as exon/intron locus tracks with the modified site marked (the primary per-category visual; built from the GTF models + the diff table, needs no long table); **(2)** the 2-panel per-site **stoichiometry** figures (`__figs_by_category/`) as in `test_diffs` (per-sample stoichiometry + pooled coverage; these require the ZN filtered long table). |
| `classify_diffs.figs_per_category` | int | `10` | Top sites (ranked by effect) to plot for each category — applies to **both** the architecture-map and the 2-panel galleries. |

**Categories** (keyed on the high-stoichiometry isoform's 3' architecture, the site's status in the longest isoform, and junction proximity): `IPA_UNIQUE`, `SPLICED_EXON_UNIQUE`, `LAST_EXON_DISTAL_ONLY`, `IPA_SHARED_EJC`, `SPLICING_EJC`, `LAST_EXON_PROXIMAL_APA_FAVORED`, `LAST_EXON_DISTAL_APA_FAVORED`, `ALTERNATIVE_LAST_EXON`, `INTERGENIC_TERMINAL_EXON`, `SHARED_TERMINAL_EXON`, `SHARED_INTERNAL_EXON`, `UNEXPLAINED_SHARED`, `HI_INTRONIC_ARTIFACT`, `UNCLASSIFIED`. `INTERGENIC_TERMINAL_EXON` captures sites in a non-IPA terminal exon that is genomically disjoint from (and ≥`intergenic_gap` away from) the comparator's terminal exon — spatially-separated / read-through alternative last exons that would otherwise fall into the vaguer `SPLICED_EXON_UNIQUE` / `UNEXPLAINED_SHARED` buckets.
...
- **Within-Bam File Tagging:**  
  - `ZN`: metagene-aware partition index  
  - `ZG`: gene index (run deterministic)  
  - `ZM`: metagene index  
  - `ZT`: string label of the form `"gene_name.gene_id.G{ZG}.T{gene-local transcript index}"`  

- **Output Summary:**  
  - `<prefix>_metrics.tsv`: isoform-level assembly metrics including exact-chain reads, truncation-assigned reads, anchor support, and the recorded assignment policy  
  ...
  - `zt_scrap/*.bam`: overlapping reads written to the scrap BAM plus per-sample overlap-resolution detail TSVs  
...
### Differential & Classification Outputs

- `results/test_diffs/{prefix}__ZN_site_diff_results.tsv`: per-site between-ZN stoichiometry test (effect = max \|Δfrac\|, `p_adj_bh`, `per_transcript_json`).
- `results/test_diffs/{prefix}__figs/`: top-K per-site figures.
- `results/test_diffs/{prefix}__ZN_site_classified.tsv`: granular structural category for each significant site (FDR<`fdr` and effect≥`min_effect`, across **all** detected `mod_code`s), with the high/low isoform identity & 3' architecture, the site's status in the high/low/anchor isoforms, junction distances, and the assigned `category`.
- `results/test_diffs/{prefix}__figs_by_category_arch/{CATEGORY}/rankNN__gene__mod__locus.png`: **isoform architecture maps** — every isoform of the gene drawn as exon/intron locus tracks (internal exons blue, terminal/3'UTR exons orange) with the modified site marked (green ○ exonic / red ✗ intronic-or-absent) and per-isoform stoichiometry + coverage labels, for the top `classify_diffs.figs_per_category` sites (by effect) of **each** category (when `classify_diffs.figures` is enabled).
- `results/test_diffs/{prefix}__figs_by_category/{CATEGORY}/rankNN__gene__mod__locus.png`: the same 2-panel per-site stoichiometry figures as `__figs/`, but rendered for the top `classify_diffs.figs_per_category` sites (by effect) of **each** structural category (when `classify_diffs.figures` is enabled).

### HTML Report — Site Classification

The report (`results/report/{prefix}_report.html`) gains a **Site Classification** section that embeds:
- a category-distribution bar chart (significant modification sites — all `mod_code`s — per structural category) and a summary table (count + %);
- definitions for every category present; and
- per category, the top `report.max_class_figs_per_category` (default 10) sites as a table (with the `mod_code` column) **and** their **isoform architecture maps** as the primary figure, with the 2-panel stoichiometry figures kept as a secondary collapsible gallery.

The number of embedded per-category figures is capped by `report.max_class_figs_per_category`; set `classify_diffs.figures: false` (or lower `figs_per_category`) to shrink the self-contained report for very large genome-wide runs.
