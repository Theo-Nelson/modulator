...
def find_overlapping_genes(read_exons, chrom, strand, gene_index):
    """
    Return list of overlapping genes on same chrom/strand with positive exonic overlap.
    """
    if not read_exons:
        return []
    ...
    candidates = []
    for g in gene_index.get((chrom, strand), []):
...
def main():
    ...
    ap.add_argument(
        "--same-strand-only",
        action="store_true",
        help="Only consider gene overlaps on the read strand (default behavior in code already)"
    )
    args = ap.parse_args()
    ...
    with pysam.AlignmentFile(args.bam, "rb") as inp, \
        ...
        for aln in inp.fetch(until_eof=True):
            ...
            overlaps = find_overlapping_genes(read_exons, chrom, strand, gene_index)
