...
def qstats(lengths: List[int]) -> Dict[str, int]:
    """Return {n, min, p25, p50, p75, p90, max} for a list of lengths."""
    if not lengths:
...
def main():
    ...
    for bam in bam_paths:
        ...
        # lengths
        total_lens: List[int] = []
        considered_lens: List[int] = []
        assigned_lens: List[int] = []
        zt_mapped_unassigned_lens: List[int] = []

        # counts in original BAM
        total_n = 0
        total_mapped = 0
        total_unmapped = 0

        # "considered" counts + failure breakdown (based on original BAM)
        considered_n = 0
        ...
        if os.path.exists(zt_bam):
            ...
            # If you didn’t write zt_tagged bams, you can’t compute assigned/unassigned here.
            assigned_n = 0

        # summarize length stats
