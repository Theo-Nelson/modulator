...
COLUMN_DEFINITIONS = {
    ...
    "strata_heterogeneous": "Heterogeneity flag: TRUE when the per-sample effects are not homogeneous (Cochran-style test, strata_heterogeneity_p < 0.05). The primary p-value is a Mantel-Haenszel/CMH COMMON-EFFECT statistic, which averages opposing per-sample effects toward 0 and reports ~null -- correct for a fixed-effect model but blind to a sign-reversing per-sample effect. When set, inspect the per-sample data rather than trusting the ~null combined p.",
    "strata_heterogeneity_p": "Count-aware test that the per-sample effects are HOMOGENEOUS (generalized-CMH decomposition: sum of single-stratum association chi2 minus the common-effect CMH ~ chi2_{(K-1)d}). Small p = the effect differs across samples (e.g. reverses sign), which the common-effect primary p averages away. Unlike a fixed effect-size threshold this scales with each sample's read count. NaN when <2 informative strata.",
