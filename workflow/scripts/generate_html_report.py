...
COLUMN_DEFINITIONS = {
    ...
    "strata_heterogeneous": "Heterogeneity flag: TRUE when the per-sample effects are not homogeneous at BH-FDR 0.05 (strata_heterogeneity_p_adj < 0.05). The primary p-value is a Mantel-Haenszel/CMH COMMON-EFFECT statistic, which averages opposing per-sample effects toward 0 and reports ~null -- correct for a fixed-effect model but blind to a per-sample effect that varies or reverses. When set, inspect the per-sample data rather than trusting the ~null combined p.",
